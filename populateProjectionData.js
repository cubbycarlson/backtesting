...
const noAction = 0;
const shelterInPlace = 1;
const projected = 2; // only after XXX - NOTE: WHAT DOES THIS MEAN?
...
function getProjections(date, state, intervention) {
  ...
  fetch(canRepoUrl + '/' + commit + '/public/data/' + state + '.' + intervention + '.json')
    ...
    .then(projectionJsonRaw => {
      ...
      fs.writeFileSync('./public/data/projections/' + date + '/' + state + '.json', JSON.stringify(projectionJson), err => {
        if (err) throw err;
...
function populateProjectionData () {
  for (let commitDate in dateToCommit.dateToCommit) {
    ...
    abbreviations.forEach(state => {
      getProjections(commitDate, state, 1) // this is always 1, ask about and fix
    })
  }
}
